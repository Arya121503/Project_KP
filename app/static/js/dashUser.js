...
function loadAssetData() {
    // Simulate loading assets (in real app, this would fetch from API)
    setTimeout(() => {
        const sampleAssets = [
            {
                id: 1,
                type: 'tanah',
                title: 'Lahan Komersial Jl. Raya Darmo',
                location: 'Darmo, Surabaya',
                area: 500,
                price: 15000000,
                status: 'available',
                details: 'Lebar Jalan 12m'
            },
            {
                id: 2,
                type: 'bangunan',
                title: 'Gedung Perkantoran Jl. HR Muhammad',
                location: 'Sukolilo, Surabaya',
                area: 800,
                price: 45000000,
                status: 'available',
                details: '3 Lantai, Parkir 20 mobil'
            },
            {
                id: 3,
                type: 'gudang',
                title: 'Gudang Industri Jl. Margomulyo',
                location: 'Benowo, Surabaya',
                area: 1200,
                price: 25000000,
                status: 'rented',
                details: 'Akses Container'
            }
        ];
        
        renderAssets(sampleAssets);
    }, 1000);
}
            ...
            <div class="col-12 text-center">
                <div class="card">
                    <div class="card-body py-5">
                        ...
                        <h4>Tidak ada aset ditemukan</h4>
            ...
            <div class="asset-card card h-100">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-start mb-3">
                        ...
                        <span class="status-${asset.status}">${getStatusLabel(asset.status)}</span>
                    </div>
                    ...
                    <p class="text-muted small">
                        <i class="fas fa-expand-arrows-alt me-1"></i>${asset.area} m² 
                        <span class="ms-3">${asset.details}</span>
                    </p>
                    <div class="price-tag mb-3">
                        Rp ${asset.price.toLocaleString('id-ID')}/bulan
                    </div>
                    <div class="d-flex gap-2">
                        <button class="btn btn-outline-danger btn-sm flex-fill" ${asset.status === 'rented' ? 'disabled' : ''}>
                            <i class="fas fa-eye me-1"></i>Detail
                        </button>
                        <button class="btn ${asset.status === 'rented' ? 'btn-outline-secondary' : 'btn-danger'} btn-sm" ${asset.status === 'rented' ? 'disabled' : ''}>
...
function applyFilters() {
    // In real app, this would filter the asset data
    console.log('Applying filters...');
    loadAssetData(); // Reload with filters
}

function resetFilters() {
    document.querySelectorAll('#assetTypeFilter, #locationFilter, #priceFilter, #statusFilter').forEach(select => {
        select.value = '';
    });
    loadAssetData();
}
